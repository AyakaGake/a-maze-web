...
export class MazeGenerator {
  ...
  constructor(mode?: string) {
    ...
    this.stateVisitedFlags = Array.from({ length: this.maze.getSize() }, () =>
      Array(this.maze.getSize()).fill(false)
    );
    this.solution = [];
  ...
  private dfSearch(state: State): boolean {
    ...
    if (
      position.x < 0 ||
      position.x >= this.maze.getSize() ||
      position.y < 0 ||
      position.y >= this.maze.getSize()
    ) {
      return false;
    }
    ...
    for (var i = 0; i < 4; i++) {
      ...
      if (
        0 <= npx &&
        npx < this.maze.getSize() &&
        0 <= npy &&
        npy < this.maze.getSize()
      ) {
